...
user1 = User.objects.create_user(username='Петр1', first_name='Петрович')
user2 = User.objects.create_user(username='Иван2', first_name='Иванович')
...
article1 = Post.objects.create(
    author=author1, categoryType='AR', title='Article 1', text='Article Text 1')
article2 = Post.objects.create(
    author=author2, categoryType='AR', title='Article 2', text='Article Text 2')
news1 = Post.objects.create(
    author=author1, categoryType='NW', title='News 1', text='News Text 1')
...
comment1 = Comment.objects.create(
    commentPost=article1, commentUser=user2, text='Comment Text 1')
comment2 = Comment.objects.create(
    commentPost=article1, commentUser=user1, text='Comment Text 2')
comment3 = Comment.objects.create(
    commentPost=article2, commentUser=user1, text='Comment Text 3')
comment4 = Comment.objects.create(
    commentPost=news1, commentUser=user2, text='Comment Text 4')
...
best_user = Author.objects.all().order_by(
    '-ratingAuthor').values('authorUser__username', 'ratingAuthor').first()
print(
    f"Лучший пользователь: {best_user['authorUser__username']} с рейтингом {best_user['ratingAuthor']}")

# Вывести дату добавления, username автора, рейтинг, заголовок и превью лучшей статьи, основываясь на лайках/дислайках к этой статье.
best_article = Post.objects.filter(
    categoryType='AR').order_by('-rating').first()
